...
$('#tabArticle').click(function() {
  ...
  $('#tabArticleContent').removeClass('hidden');
  $('#tabCategoryContent').addClass('hidden');
});

$('#tabCategory').click(function() {
  $(this).addClass('text-blue-600 border-b-2 border-blue-600 text-gray-800').removeClass('text-gray-500');
  $('#tabArticle').removeClass('text-blue-600 border-b-2 border-blue-600 text-gray-800').addClass('text-gray-500');
  $('#tabCategoryContent').removeClass('hidden');
  $('#tabArticleContent').addClass('hidden');
});
...
// 提交文章
$('#addArticleForm').on('submit', async (e) => {
  e.preventDefault();
  ...
  try {
    ...
    const maxIdRes = await fetch('http://localhost:5050/articles?_sort=id&_order=desc&_limit=1');
    const maxIdData = await maxIdRes.json();
    const nextId = maxIdData.length > 0 ? maxIdData[0].id + 1 : 1;
    const link = `article_${nextId}.html`;
